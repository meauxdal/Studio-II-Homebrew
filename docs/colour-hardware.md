...
`games/pacman/studio-ii/pacman.asm` gains a `$A00` page holding a 15-instruction loop and a
...
```sh
python build.py pacman/studio-ii
```

Three portability fixes were needed and are included: `asmx` is K&R-era C that
modern clang rejects without the compatibility flags now used by `build.py`.
The legacy `s9tobinary.py` and `makest2.py` scripts were also ported to Python 3
on the upstream-compatible branch. With those fixes,
