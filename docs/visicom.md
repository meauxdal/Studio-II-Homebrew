...
`games/pacman/visicom/` is a second colour port of Pacman, to a machine that is
often described as a Studio II relative and is not one. It is a separate source
tree, not a build option: `games/pacman/studio-ii/` is untouched and still produces the
byte-identical Studio II / CDP1864 cartridge described in [Colour hardware](colour-hardware.md).
...
pills, from the maze bytes at `$1000-$103B`). `games/pacman/visicom/checkplane1.py`
...
```sh
Vtop --machine visicom --bios visicom.rom --cart build/pacman/visicom/pacman.st2 \
