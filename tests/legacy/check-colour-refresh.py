...
def hockey(bios, folder='hockey/visicom', revision='197ba8f'):
    path='Games/HockeyVisicom/hockey.st2'
    old,new=original(path,revision),(ROOT/'build'/folder/'hockey.st2').read_bytes()
    sy=symbols(ROOT/'build'/folder/'hockey.lst')
    cases=0
    for game in range(4):
      for height in (3,5):
...
def pacman(bios):
    ...
    old,new=original(path),(ROOT/'build/pacman/visicom/pacman.st2').read_bytes()
    cases=0
...
def invaders():
    ...
    a,b=original(path),(ROOT/'build/invaders/studio-ii/invaders.st2').read_bytes()
